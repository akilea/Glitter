...
"void main()\n"
"{\n"
"color = vec4(0.1f, 1.0f, 0.2f, 1.0f);\n"
"}\n\0";
...
int main(int argc, char * argv[]) {
    ...
    gladLoadGL(); //Load open GL on dirait...
    fprintf(stderr, "OpenGL %s\n", glGetString(GL_VERSION));

    // Define the viewport dimensions
    int width, height;
    glfwGetFramebufferSize(mWindow, &width, &height);
    glViewport(0, 0, width, height); //Map le clip space au screen space
...
void onInit()
{
    ...
    //on supprime les shaders des qu'on a a plus besoin
    glDeleteShader(vertexShader);
    glDeleteShader(fragmentShader);


    // --------------Set up vertex data (and buffer(s)) and attribute pointers------------------
    GLfloat verticesCenter[] = {
        -0.5f, -0.5f, 0.0f, // Left  
    ...
    glGenVertexArrays(1, &VAO); //Vertex Array Object - VBO + attributes pointers
    glGenBuffers(1, &VBO);
